...
def bureau_and_balance(num_rows = None, nan_as_category = True):
    ...
    for col in bb_cat:
        bb_aggregations[col] = ['mean', 'median']
    ...
    num_aggregations = {
        ...
        'AMT_CREDIT_SUM': ['max', 'mean', 'median', 'sum'],
        'AMT_CREDIT_SUM_DEBT': ['max', 'mean', 'median', 'sum'],
        'AMT_CREDIT_SUM_OVERDUE': ['mean', 'median'],
        'AMT_CREDIT_SUM_LIMIT': ['mean', 'median', 'sum'],
        'AMT_ANNUITY': ['max', 'mean', 'median'],
        'CNT_CREDIT_PROLONG': ['sum'],
        'MONTHS_BALANCE_MIN': ['min'],
        'MONTHS_BALANCE_MAX': ['max'],
        'MONTHS_BALANCE_SIZE': ['mean', 'median', 'sum']
    }
    # Bureau and bureau_balance categorical features
    cat_aggregations = {}
    for cat in bureau_cat: cat_aggregations[cat] = ['mean', 'median']
    for cat in bb_cat: cat_aggregations[cat + "_MEAN"] = ['mean', 'median']
...
def previous_applications(num_rows = None, nan_as_category = True):
    ...
    num_aggregations = {
        ...
        'APP_CREDIT_PERC': ['min', 'max', 'mean', 'median', 'var'],
...
def pos_cash(num_rows = None, nan_as_category = True):
    ...
    # Features
    aggregations = {
        'MONTHS_BALANCE': ['max', 'mean', 'median', 'size'],
        'SK_DPD': ['max', 'mean', 'median'],
        'SK_DPD_DEF': ['max', 'mean', 'median']
    }
    for cat in cat_cols:
        aggregations[cat] = ['mean', 'median']
...
def installments_payments(num_rows = None, nan_as_category = True):
    ...
    aggregations = {
        ...
        'PAYMENT_PERC': ['max', 'mean', 'median', 'sum', 'var'],
        'PAYMENT_DIFF': ['max', 'mean', 'median', 'sum', 'var'],
...
def credit_card_balance(num_rows = None, nan_as_category = True):
    ...
    cc_agg = cc.groupby('SK_ID_CURR').agg(['min', 'max', 'mean', 'median', 'sum', 'var'])
...
def BuildDataFromZipFile(lGBMclf, BestThreshold):
    # Loading the zipFile
    if not os.path.isdir('input'):
        print("Getting zip File from URL")
        request = requests.get("https://s3-eu-west-1.amazonaws.com/static.oc-static.com/prod/courses/files/Parcours_data_scientist/Projet+-+Impl%C3%A9menter+un+mod%C3%A8le+de+scoring/Projet+Mise+en+prod+-+home-credit-default-risk.zip")
        print("Unzip")
    ...
    df = preprocess()
    #df.to_csv("featureDF.csv", index= False)
    #df = pd.read_csv("FeatureDF.csv", nrows=10000, dtype=dtypesR)
    #df.info()
    ...
    print("Predicting")
    YProba = lGBMclf.predict_proba(trainXDF)

    DashBoardDF['Proba'] = YProba[:,1]
    DashBoardDF['Predict'] = YProba[:,1] > BestThreshold
...
def main(debug = False):
    ...
    df = preprocess(num_rows)
    print("Final DF size", df.shape)
    print("Saving CSV file")
    df.to_csv("featureDFx.csv", index= False)
    print(df.shape)
    print(df.info())
